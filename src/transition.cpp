...
void Transition::perform() {
    ...
    end_state_p_->pose = start_state_p_->pose;

    // TODO: Communicate with PX4 and get errors (if any) and set them in the error code

    const std::type_info& start_state_type_info = typeid(start_state_p_.get());
    const std::type_info& end_state_type_info = typeid(end_state_p_.get());
